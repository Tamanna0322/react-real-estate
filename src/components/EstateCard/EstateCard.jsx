...
const EstateCard = ({estateCard}) => {
    return (
        <div className='border border-[#0056b3] border-dotted p-3 rounded-lg'>
           ...
           <p className='text-primary mt-2 font-semibold'>#{estateCard.segment_name}</p>
           ...
           <Link to={`/estateCard/${estateCard.id}`}><button className='btn w-full bg-blue-800 text-white'>View Property</button></Link>
        </div>
    );
};
